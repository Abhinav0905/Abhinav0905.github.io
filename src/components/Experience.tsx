const experiences = [
  ...
  },
  {
    ...
    description: [
      'Product Development: Legacy product: two features: (a) Escalation management: Developed Java Spring Boot REST APIs and an Angular UI to enable Walgreens and its distributors to configure exception rules in real time. Empowered users to define alerts on the fly—cutting resolution time by **40%** and reducing operational losses (support staffing and logistics) by **$1M per year**. (b) Automated real-time inventory & order synchronization: Developed a Java Spring Boot public API to integrate RfXcel\'s platform with Manufacturer (AbbVie) SAP system—automating **100%** of order handoffs, eliminating manual, error-prone updates, ensuring **100% accurate**, up-to-date inventory and order status, and saving **$200K per year** in labor costs.',
      ...
      'Internal Productivity: Leveraged GenAI (LLM Models) to prototype and launch several tools for internal productivity automation such as Text2SQL Query Assistant: Built a GenAI-powered tool using OpenAI\'s API (GPT-4) and AWS Redshift to translate natural-language prompts into SQL queries for Customer Support Analysts. Reduced support-ticket processing time by **45%**, reduced the dependency to hire three contractor roles, and saved **$100K annually**. Automated Release-Notes Generator: Developed a pipeline leveraging the JIRA API, Confluence data, and OpenAI to draft customer-friendly release notes automatically at the end of each sprint. Saved **576 Engineering Working hours per year** and increased customer satisfaction by **65%**.',
      'Gen AI based Project Management: Leveraged GenAI to build a conversational Jira assistant—enabling natural-language ticket queries, automated intent classification, and escalation-risk scoring from ticket comments; reduced average triage time by **45%**, prevented **~20 escalations/month**, and saved **~16 engineer-hours per week** (**~832 hours/year**), cutting support costs by **~$120K annually**.'
    ]
  },
  {
